# vgi-conform — dev and test targets.
#
# Usage:
...
#
# test-sql is self-contained: it points VGI_CONFORM_WORKER at the worker run as
...
WORKER_STDIO    ?= uv run --python 3.13 conform_worker.py
...
test-sql:
	PATH="$(HAYBARN_BIN):$$PATH" \
		VGI_CONFORM_WORKER="$(WORKER_STDIO)" \
