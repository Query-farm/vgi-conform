# CI: the vgi-conform worker integration suite

[`.github/workflows/ci.yml`](../.github/workflows/ci.yml) runs the unit tests
and this repo's sqllogictest suite (`test/sql/*.test`) against the vgi-conform
...
1. **Install the worker** — `uv sync --frozen` into a venv. `conform_worker.py`
   is a self-contained PEP 723 stdio worker the extension can spawn via
   `uv run conform_worker.py`; its inline deps are resolved from PyPI. The
   worker is pure/offline — no model or data downloads.
...
4. **Run** — [`run-integration.sh`](run-integration.sh) stages the preprocessed
   tree, points `VGI_CONFORM_WORKER` at `uv run conform_worker.py`, warms the
...
VGI_CONFORM_WORKER="uv run --python 3.13 conform_worker.py" \
  ci/run-integration.sh
```

Or use the Makefile target `make test-sql`, which installs `haybarn-unittest`
as a uv tool and points the worker at `uv run --python 3.13 conform_worker.py`.
