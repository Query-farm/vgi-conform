...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-conform
...
# ATTACH (`${VGI_CONFORM_WORKER}`):
#
#   subprocess : a bare stdio command (`uv run conform_worker.py`) — the extension
...
#                        `uv run --python 3.13 <repo>/conform_worker.py`.
# Optional:
...
WORKER_CMD="${WORKER_CMD:-uv run --python 3.13 $REPO/conform_worker.py}"
...
# ---------------------------------------------------------------------------
# Per-transport: resolve VGI_CONFORM_WORKER (the LOCATION) and, for the out-of-band
...
case "$TRANSPORT" in
  subprocess)
    export VGI_CONFORM_WORKER="$WORKER_CMD"
    ;;
  ...
  http)
    ...
    PORT_FILE="$(mktemp -u "${TMPDIR:-/tmp}/conform-port.XXXXXX")"
    LOG_FILE="${TMPDIR:-/tmp}/conform-http-server.log"
    ...
    export VGI_CONFORM_WORKER="http://127.0.0.1:$PORT"
    ;;

  unix)
    # Boot the worker bound to an AF_UNIX socket; poll for the socket file.
    SOCK="${TMPDIR:-/tmp}/conform-$$.sock"
    rm -f "$SOCK"
    LOG_FILE="${TMPDIR:-/tmp}/conform-unix-server.log"
    ...
    export VGI_CONFORM_WORKER="unix://$SOCK"
    ;;
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_CONFORM_WORKER) ..."
OUT_FILE="$(mktemp)"
set +e
