...
#   VGI_CONFORM_WORKER worker LOCATION the .test files ATTACH (a stdio command
#                      such as `uv run conform_worker.py`, or an http:// URL)
# Optional:
#   STAGE             scratch dir for the preprocessed test tree (default: mktemp)
set -euo pipefail

: "${HAYBARN_UNITTEST:?path to the haybarn-unittest binary}"
: "${VGI_CONFORM_WORKER:?worker LOCATION (stdio command or http:// URL)}"
...
echo "Running suite (worker: $VGI_CONFORM_WORKER) ..."
